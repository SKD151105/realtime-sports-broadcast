...
export const MatchCard: React.FC<MatchCardProps> = ({ match, isActive, onWatch, onUnwatch }) => {
  ...
  return (
    <div className={`
      ...
      ${isActive ? 'shadow-hard translate-x-[-2px] translate-y-[-2px] ring-2 ring-lime-300 ring-offset-2' : 'hover:shadow-hard-sm'}
    `}>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <div className="flex justify-between items-center">
          <span className="font-bold text-lg text-zinc-900 line-clamp-1">{match.homeTeam}</span>
          <span
            className={`
              font-bold text-2xl border border-black rounded-lg px-3 py-1 min-w-[3rem] text-center transition-colors
              ${homePulse ? 'bg-lime-200 animate-pulse' : 'bg-gray-100'}
            `}
          >
            {match.homeScore}
          </span>
        ...
        <div className="flex justify-between items-center">
          <span className="font-bold text-lg text-zinc-900 line-clamp-1">{match.awayTeam}</span>
          <span
            className={`
              font-bold text-2xl border border-black rounded-lg px-3 py-1 min-w-[3rem] text-center transition-colors
              ${awayPulse ? 'bg-lime-200 animate-pulse' : 'bg-gray-100'}
            `}
          >
            {match.awayScore}
          </span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`
              ...
              ${isActive 
                ? 'bg-sky-200 text-black cursor-default opacity-100' 
                : 'bg-lime-300 text-black hover:bg-lime-200 active:translate-y-0.5'
              }
            `}
          >
            {actionLabel}

    ...
    try {
      ...
      socket.onopen = () => {
        ...
        heartbeatInterval.current = setInterval(() => {
          ...
          const now = Date.now();
          if (lastSeenAt.current && now - lastSeenAt.current > HEARTBEAT_TIMEOUT_MS) {
            // Force a close to trigger reconnect + UI status update.
            try {
              ws.current.close();
            } catch {
              // ignore
      ...
      socket.onerror = (event) => {
        ...
        // We rely on onclose + heartbeat to update status.

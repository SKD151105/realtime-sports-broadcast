...
const App: React.FC = () => {
  ...
  const activeMatch = useMemo(() => {
    ...
    const activeId = String(activeMatchId);
    return matches.find((m) => String(m.id) === activeId) ?? null;
  }, [matches, activeMatchId]);

  const feedMode = activeMatch?.status?.toLowerCase() === 'finished' ? 'recap' : 'live';

  return (
    <div className="min-h-screen p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-lime-300 border-2 border-black rounded-2xl p-6 shadow-hard">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-zinc-900 mb-1">
              Spotrz
            </h1>
            <p className="text-sm font-medium opacity-80">Live scores and commentary</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <main className="lg:col-span-2 space-y-6">
            ...
            {newMatchesCount > 0 && (
              <div className="flex items-center justify-between gap-3 bg-lime-300 border-2 border-black rounded-xl px-4 py-3 shadow-hard-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pagedMatches.map((match) => (
                <MatchCard 
                  key={match.id} 
                  match={match} 
                  isActive={activeMatchId != null && String(activeMatchId) === String(match.id)}
                  onWatch={watchMatch}
